    ...
    <div class="max-w-screen-xl px-4 py-12 mx-auto sm:px-6 lg:px-8">
        <div class="grid items-start grid-cols-1 gap-8 md:grid-cols-2">
            ...
            <div>
                ...
                <article class="my-4 text-gray-700">
                    {!! $this->product->translateAttribute('description') !!}
                </article>

                <div>
                    <table class="table-auto">
                        <caption class="text-lg font-bold py-2 mb-2 text-left">Specifications</caption>
                        <tbody>
                            <tr class="border-t border-b">
                                <td class="py-2 px-4 text-left font-bold">Product Type</td>
                                <td class="py-2 px-4 text-right">{{ $this->product->translateAttribute('product_type') }}</td>
                            </tr>

                            <tr class="border-t border-b bg-gray-50">
                                <td class="py-2 px-4 text-left font-bold border-y">Weight</td>
                                <td class="py-2 px-4 text-right border-y">
                                    <span>{{ number_format($this->product->translateAttribute('weight')) }}</span>
                                    <span>{{ $this->product->translateAttribute('unit') }}</span>
                                </td>
                            </tr>

                            <tr class="border-t border-b">
                                <td class="py-2 px-4 text-left font-bold">Purity</td>
                                <td class="py-2 px-4 text-right">{{ $this->product->translateAttribute('purity') }}‰</td>
                            </tr>

                            <tr class="border-t border-b bg-gray-50">
                                <td class="py-2 px-4 text-left font-bold">Metal</td>
                                <td class="py-2 px-4 text-right">{{ $this->product->translateAttribute('metal') }}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
